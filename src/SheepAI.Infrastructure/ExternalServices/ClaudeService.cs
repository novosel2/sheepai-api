...
public sealed class ClaudeService : IClaudeService
{
    ...
    private static BetaMsg.BetaTool BuildRespondTool()
    {
        ...
        return new BetaMsg.BetaTool
        {
            ...
            Description =
                ...
                "When returning a contact widget, also return a map widget for that contact's office location if coordinates are available in the documents. " +
                "When a contact widget is included, do NOT repeat the name, phone, or email in the text response — those details will be shown in the card.",
        ...
        CancellationToken ct = default)
    {
        ...
        const string systemPromptText =
            ...
            "Ako koristiš contact widget, NE ponavljaj ime, telefon ni email u tekstu odgovora — ti podaci bit će prikazani u kartici. " +
            "Ako taj kontakt ima poznatu lokaciju ili adresu ureda u dokumentima, dodaj i map widget za tu istu lokaciju. " +
